...
from main_utils_1 import *
# from main_functions import *
from main_feature_functions import get_adv_features, get_features, get_peak_stat_features
...
def _ecg_features(ecgDF, ecg_rt=512.):
    ...
    hrv_feat, ecg_hrv_cols = get_adv_features(ecg.values, sampling_rate=ecg_rt)

    _, info = nk.ecg_peaks(ecg.values,
                            sampling_rate=ecg_rt,
                            correct_artifacts=True)

    pyhrv_rpeaks, = correct_rpeaks(signal=ecg.values,
                                    rpeaks=info['ECG_R_Peaks'],
                                    sampling_rate=ecg_rt,
                                    tol=0.05)

    _, pyhrv_rpeaks = extract_heartbeats(signal=ecg.values,
                                        rpeaks=pyhrv_rpeaks,
                                        sampling_rate=ecg_rt,
                                        before=0.2,
                                        after=0.4)
...
def _eda_features(edaDF, eda_rt=128.):
    ...
    _, info = nk.eda_process(edaDF['GSR Conductance CAL'], sampling_rate = eda_rt)
    ...
    stat_col_ph = ['ph_mean_features', 'ph_std_features', 'ph_min_features', 'ph_max_features', 'ph_skew_features',
                    'ph_kurtosis_features', 'ph_median_features', 'ph_entropy_features',
                    'ph_iqr_features', 'ph_area_ts', 'ph_sq_area_ts', 'ph_mad_ts']

    phasic = get_features(phasic)
    phasic = pd.DataFrame([phasic], columns=stat_col_ph)

    stat_col_ton = ['ton_mean_features', 'ton_std_features', 'ton_min_features', 'ton_max_features', 'ton_skew_features',
                    'ton_kurtosis_features', 'ton_median_features', 'ton_entropy_features',
                    'ton_iqr_features', 'ton_area_ts', 'ton_sq_area_ts', 'ton_mad_ts']
